...
import { addOrganization } from '../../slices/adminSlice';

const AddOrg = ({ addRef, showAddOrg, setShowAddOrg }) => {
    const [org, setOrg] = useState({})
    const [vendorError, setVendorError] = useState({});
    const dispatch = useDispatch();

    useEffect(() => {
        document.body.style.overflow = showAddOrg ? "hidden" : "auto";
    }, [showAddOrg]);
    ...
    return (
        <div className='fixed flex justify-center items-center w-screen h-screen z-50 bg-black bg-opacity-50'>
            <div ref={addRef} className="bg-white rounded-lg">
                ...
                <form className='flex flex-col' onSubmit={AddOrg} action="">
